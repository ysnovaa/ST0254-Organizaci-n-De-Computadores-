...
class CompilationEngine:
    ...
    def compile_class(self):
        ...
        jyy_class = CompilationTypes.JyyClass(class_name)

        self.tokenizer.advance() # {

        self.compile_class_vars(jyy_class)
        self.compile_class_subroutines(jyy_class)
    ...
    def compile_class_vars(self, jyy_class):
        ...
        while token is not None and token.type == 'keyword' and\
            ...
            while still_vars:
                ...
                if is_static:
                    jyy_class.add_static(var_name, var_type)
                else:
                    jyy_class.add_field(var_name, var_type)
    ...
    def compile_class_subroutines(self, jyy_class):
        ...
        while token is not None and token.type == 'keyword'\
                and token.value in ['constructor', 'funcion', 'metodo']:
            ...
            jyy_subroutine = CompilationTypes.JyySubroutine(
                    name, subroutine_type, return_type, jyy_class
                )

            self.tokenizer.advance() # ( - open parameterList

            self.compile_parameter_list(jyy_subroutine)

            self.tokenizer.advance() # ) - close parameterList

            self.compile_subroutine_body(jyy_subroutine)
    ...
    def compile_parameter_list(self, jyy_subroutine):
        ...
        while still_vars:
            ...
            jyy_subroutine.add_arg(param_name, param_type)
    ...
    def compile_subroutine_body(self, jyy_subroutine):
        ...
        self.compile_subroutine_vars(jyy_subroutine)

        self.vm_writer.write_function(jyy_subroutine)

        if jyy_subroutine.subroutine_type == 'constructor':
            field_count = jyy_subroutine.jyy_class.field_symbols
            self.vm_writer.write_push('constant', field_count)
            self.vm_writer.write_call('Memory', 'alloc', 1)
            # Set 'this' in the function to allow it to return it
            self.vm_writer.write_pop('pointer', 0)
        elif jyy_subroutine.subroutine_type == 'metodo':
            self.vm_writer.write_push('argument', 0)
            self.vm_writer.write_pop('pointer', 0)

        self.compile_statements(jyy_subroutine)
    ...
    def compile_subroutine_vars(self, jyy_subroutine):
        ...
        while token is not None and token == ('keyword', 'variable'):
            self.tokenizer.advance()
            # var_type
            var_type = self.tokenizer.advance().value
            # var_name
            ...
            jyy_subroutine.add_var(var_name, var_type)
    ...
    def compile_statements(self, jyy_subroutine):
        ...
        while check_statements:
            ...
            if token == ('keyword', 'si'):
                self.compile_statement_if(jyy_subroutine)
            elif token == ('keyword', 'mientras'):
                self.compile_statement_while(jyy_subroutine)
            elif token == ('keyword', 'permite'):
                self.compile_statement_let(jyy_subroutine)
            elif token == ('keyword', 'Sistema'):
                self.compile_statement_do(jyy_subroutine)
            elif token == ('keyword', 'retorne'):
                self.compile_statement_return(jyy_subroutine)
            else:
    ...
    def compile_statement_if(self, jyy_subroutine):
        ...
        self.compile_expression(jyy_subroutine)
    ...
    def compile_statement_let(self, jyy_subroutine):
        ...
        jyy_symbol = jyy_subroutine.get_symbol(var_name)

        is_array = self.tokenizer.current_token().value == '['
        if is_array:
            self.tokenizer.advance() # [
            self.compile_expression(jyy_subroutine) # Index
            self.tokenizer.advance() # ]
            self.tokenizer.advance() # =
            # Add the base and index
            self.vm_writer.write_push_symbol(jyy_symbol)
            ...
            # pop it yet
            self.compile_expression(jyy_subroutine) # Expression to assign
            ...
            self.tokenizer.advance() # =
            self.compile_expression(jyy_subroutine) # Expression to assign
            self.vm_writer.write_pop_symbol(jyy_symbol)
    ...
    def compile_term(self, jyy_subroutine):
        ...
        # In case of a function call or variable name
            ...
            token_var = jyy_subroutine.get_symbol(token_value)
            ...
            if token.value == '[': # Array
                ...
                func_class = jyy_subroutine.jyy_class.name
                ...
                if token.value == '.':
                    ...
                    func_obj = jyy_subroutine.get_symbol(token_value)
                ...
                if token.value == '(':
                    ...
                    self.tokenizer.advance() # (
                    arg_count += self.compile_expression_list(jyy_subroutine)
